...
export const cartReducer = (state: Cart, action: { type: string, payload?: any }): Cart => {
  if (action.type === CartActionType.ADD_TO_CART) {
    ...
    if (existingProduct) {
      ...
      return { products: updatedProduct, subTotal: state.subTotal + product.currentPrice, itemsNum: state.itemsNum + 1 };
    } else {
      return {
        ...state,
